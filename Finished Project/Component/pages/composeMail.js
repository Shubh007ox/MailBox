...
function ComposeMail() {
  ...
  const MailSendHandler = (event) => {
    ...
    localStorage.setItem("email", JSON.stringify(enteredMail));
    const SendM = localStorage.getItem('email')
    const SendMail = SendM.replace("@","");
    ...
    if (enteredMail.trim().length === 0) {
      ...
      const resData = () => {
        ...
        sendRequest(
          {
            request: "post",
            url: `https://authentication-61603-default-rtdb.firebaseio.com/sent${enterM}.json`,
            data: objectData,
      ...
      sendRequest(
        {
          request: "post",
          url: `https://authentication-61603-default-rtdb.firebaseio.com/receive${SendMailFinal}.json`,
          data: objectData,
          header: { "Content-type": "application/json" },
        },
        resData
